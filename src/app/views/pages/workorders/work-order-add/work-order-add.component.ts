...
export class WorkOrderAddComponent implements OnInit {
    ...
    ngOnInit(): void {
        //temp to reoslve hanging issue
        if (!sessionStorage.getItem("isLoaded") || sessionStorage.getItem("isLoaded") != "true") {
    ...
    customSubmit(form, isDraft = false) {
        ...
        if (this.formInterface.object.workoderId && isDraft) {
        ...
        if (this.hasCase)
            this.formInterface.section.backUrl = '/cases/cases/case-view/' + this.formInterface.object.caseId + '/workorders'
        ...
        this.formInterface.onSubmit().then((res) => {
            console.log(res);
        })
    }
