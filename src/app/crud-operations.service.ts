...
export class CrudOperationsService {
    ...
    async loadDependencies(owner = null) {
        try {
            ...
            await Promise.all([...Object.keys(this.section.pullDependecies).map(async (key) => {
                let url = this.utility.baseUrl + key + (owner ? '?resourceOwner=' + owner : '');
    ...
    async onSubmit(hasForm = true, noBackground = false) {
        ...
        try {
            ...
            if (noBackground) {
                try {
                    this.isEditMode ? await this.dataService.putData(this.utility.baseUrl + this.section.module, JSON.stringify(this.object), true).toPromise() :
                        await this.dataService.postData(this.utility.baseUrl + this.section.module, JSON.stringify(this.object), true).toPromise();
                    return this.object;
                } catch (error) {
                    return error;
                }
                ...
                swal(this.section.module.toString().slice(0, -1) + " Data Added for Processing, Check Uploads Tab for Status")
                if (hasForm) {
                    if (this.section.module != "tasks") this.router.navigate([this.section.backUrl])
                } else
                    return this.object;
            }
            return true
